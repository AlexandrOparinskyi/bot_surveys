...
@faq_router.message(Command(commands="faq_raif"))
...
@faq_router.message(Command(commands="faq_vtb"))
async def get_faq(message: Message, session: AsyncSession):
    faq = await session.scalars(select(FAQVtb))
    keyboard = create_faq_vtb_keyboard(faq)
    await message.answer("<b>Часто задаваемые вопросы</b>\n\n"
                         "Выберите вопрос, который вас интересует",
                         reply_markup=keyboard)


@faq_router.callback_query(F.data.startswith("detail_vtb_"))
async def get_detail_vtb(callback: CallbackQuery,
                         session: AsyncSession,
                         bot: Bot):
    _, _, faq_id = callback.data.split("_")
    faq = await session.scalar(select(FAQVtb).where(FAQVtb.id == int(faq_id)))
    if faq.file_path_1:
        file_path = os.path.join(DATA_FOLDER, faq.file_path_1)
        _, file_exp = file_path.split(".")
        document = FSInputFile(file_path)
    ...
    if faq.file_path_3:
        ...
        if file_exp in ["jpeg", "jpg", "png", "webp", "gif"]:
            ...
            await bot.send_document(callback.from_user.id, document)
    if faq.file_path_4:
        file_path = os.path.join(DATA_FOLDER, faq.file_path_4)
        _, file_exp = file_path.split(".")
        document = FSInputFile(file_path)
        ...
        if file_exp in ["jpeg", "jpg", "png", "webp", "gif"]:
            ...
            await bot.send_document(callback.from_user.id, document)
    if faq.file_path_5:
        file_path = os.path.join(DATA_FOLDER, faq.file_path_5)
        _, file_exp = file_path.split(".")
        document = FSInputFile(file_path)
